...
const WORD = "function";
const HIGHLIGHT_FRAMES = [60, 100, 140];

export const Shortcut3: React.FC<{ duration: number }> = ({ duration }) => {
  ...
  const highlightCount = HIGHLIGHT_FRAMES.filter((f) => frame >= f).length;
        ...
        <div className="flex flex-col items-center gap-10">
          ...
          >
            ...
            <p className="font-bold text-zinc-400 text-center" style={{ fontSize: 34, maxWidth: 800 }}>
              Pilih kata berikutnya — rename semua sekaligus
            </p>
          </div>

          {frame >= 60 && (
            <div className="flex flex-col gap-3 rounded-2xl p-6" style={{ background: "#1e1e1e", border: "1px solid #333", minWidth: 700 }}>
              {["const", "let", "var"].map((line, i) => (
                <div key={line} className="flex items-center gap-3 font-mono" style={{ fontSize: 36 }}>
